...
#define NUM_VERTS       (NUM_SLICES+2)  // base points + base center + apex
    ...
    : MeshInfo(GL_TRIANGLES)
{
  ...
  // initialiize other mesh properties
...
Instance *MakeCone (cs237::vec3f pos, cs237::vec3f dir, float angle, float height)
{
  ...
  // the cone is already transformed to world coordinates, so the transforms are all I
