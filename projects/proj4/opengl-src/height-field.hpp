...
class HeightField {
  ...
  //! \param cmap the color texture for the ground
  //! \param nmap the normal-map texture for the ground (or nullptr)
    HeightField (
  ...
  //! return the diffuse-color map for the ground
    cs237::texture2D *DiffuseMap () const { return this->_colorMap; }

  //! return the normal map for the ground
    cs237::texture2D *NormalMap () const { return this->_normMap; }
  ...
  private:
    ...
    const cs237::color3f _color;        //!< the color of the ground in wireframe, flat-shading,
                                        //!  or diffuse mode
    cs237::texture2D    *_colorMap;     //!< the color texture for the ground in texturing and
                                        //!  normal-mapping modes
    cs237::texture2D    *_normMap;      //!< the normal-map texture for the ground in
                                        //!  normal-mapping mode.
