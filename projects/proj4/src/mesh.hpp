...
/* CMSC23700 Project 4 sample code (Autumn 2022)
 *
...
struct TextureProperty {
    ...
    void define (Proj4 *app, cs237::Image2D *img);

    void define (Proj4 *app, std::string const &name)
    {
...
struct Mesh {
    ...
    Mesh (Proj4 *app, OBJ::Model const *model, int grpId);

    /// create a Mesh object by triangulating a height field
    /// \param app    the owning app
    /// \param hf     the height-field
    Mesh (Proj4 *app, HeightField const *hf);
...
public:
    ...
    MeshFactory (Proj4 *app, int nMeshes);
...
private:
    Proj4 *_app;                        ///< the owning application
