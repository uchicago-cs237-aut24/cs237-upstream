...
void Proj5Window::_initMeshes (const Scene *scene)
{
    Proj5 *app = reinterpret_cast<Proj5 *>(this->_app);
...
void Proj5Window::_initForwardRenderInfo ()
{
    ...
    /* create the pipeline layouts for the wire-frame renderer */
    {
...
void Proj5Window::_initDescriptorPools ()
{
    auto device = this->device();
...
void Proj5Window::_initLighting ()
{
...
void Proj5Window::_invalidateFrameUBOs ()
{
    // mark the per-frame UBOs as being out-of-sync with the cache
    for (auto it : this->_frames) {
        reinterpret_cast<Proj5Window::FrameData *>(it)->valid = false;
    }

    /** HINT: update the cache for per-frame data */
}

void Proj5Window::_recordForwardCommands (Proj5Window::FrameData *frame)
{
    ...
    /*** BEGIN COMMANDS ***/
    {
        ...
        case RenderMode::eTextured:
            ...
            break;
        default:
            ERROR("Proj5Window::_recordForwardCommands: invalid render mode");
        } /* switch */
...
void Proj5Window::draw ()
{
/** HINT: you will need to restructure this function for the situation where
 ** rain is enabled.  You may find it useful to split it into two (or more functions)
 ** where the code below is used when rain is disabled and a different code path
 ** is used when rain is enabled.
 **/
    ...
    auto frame = reinterpret_cast<Proj5Window::FrameData *>(this->_currentFrame());
...
void Proj5Window::reshape (int wid, int ht)
{
...
void Proj5Window::key (int key, int scancode, int action, int mods)
{
...
/** HINT: uncomment when deferred rendering is enabled */
//                this->_renderFlags.mode = RenderMode::eDeferred;
                break;
            ...
            case GLFW_KEY_H:  // 'h' or 'H' ==> display help message
                reinterpret_cast<Proj5 *>(this->_app)->controlsHelpMessage();
                break;
            case GLFW_KEY_R:  // 'r' or 'R' ==> toggle the rain
                reinterpret_cast<Proj5 *>(this->_app)->toggleRain();
                break;
...
/* virtual */
cs237::Window::FrameData *Proj5Window::_allocFrameData (cs237::Window *w) /* override */
{
    return new Proj5Window::FrameData (w);
}

Proj5Window::FrameData::FrameData (cs237::Window *w)
: cs237::Window::FrameData(w)
{
    auto win = reinterpret_cast<Proj5Window *>(w);
...
/* virtual */
Proj5Window::FrameData::~FrameData ()
{
