...
/// A container for information about the rain particle system
struct Rain {
    uint32_t numParticles;
    // generator rectangle
    vk::Extent2D genExtent;     ///< size of generator rectangle
    glm::vec3 genOrigin;        ///< world-space origin of generator rectangle
    glm::vec3 genTan;           ///< unit-length tangent vector for generator rectangle
    glm::vec3 genBitan;         ///< unit-length bitangent vector for generator rectangle
    // death plane
    cs237::Planef_t deathPlane; ///< "death" plane that defines the bounds of the
                                ///  particle system
    // particle properties
    glm::vec3 initialVelocity;  ///< the initial velocity vector for generated particles
    glm::vec3 acceleration;     ///< the acceleration vector
    glm::vec4 color;            ///< the color of a particle
};
...
class Scene {
  public:
    ...
    /// get information about the rain particle system
    const Rain & rain () const { return this->_rain; }

  private:
    bool _loaded;               ///< has the scene been loaded?

    /* view */
    uint32_t _wid;              ///< initial window width
    uint32_t _ht;               ///< initial window height
    ...
    Rain _rain;                 ///< information about the rain simulation
